// app/private/UserProfile/[userId].tsx
...
export default function UserProfile() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setUser(userData);
        const { data: listingsData } = await ListingsService.getListingsByUserId(userId);
  ...
  const renderStar = (i: number) => (
    <Ionicons
      key={i}
      name={i < Math.round(user.averageRating) ? 'star' : 'star-outline'}
      size={20}
      color="#f1c40f"
      style={{ marginRight: 4 }}
    />
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.bio}>{user.bio || "Pas de bio."}</Text>
        ...
        {connectedUserId !== user._id && (
          <TouchableOpacity
            style={styles.messageButton}
             onPress={() => router.push({ pathname: '/private/chat/[id]', params: { id: user._id } })}
          >
        ...
        <View style={styles.section}>
          ...
          {listings.length === 0 ? (
            <Text style={styles.noListings}>pas de bio</Text>
          ) : (
            <FlatList
              data={listings}
...
const styles = StyleSheet.create({
  ...
  backButton: { position: 'absolute', top: HEADER_HEIGHT - 40, left: 16, zIndex: 1 },
  avatar: {
  ...
  editButton: {
    ...
    backgroundColor: '#0e58b3', paddingVertical: 6, paddingHorizontal: 12, borderRadius:20
  },
  ...
  messageButtonText: { color:'#fff', fontWeight:'500', marginLeft:8 },
   ...
   listingCard: {
        ...
        padding: 8, marginBottom: 12, elevation: 2, width: '100%'
    },
    listingImage: { width: 60, height: 60, borderRadius: 8 },
    listingPlaceholder: {
        width: 60, height: 60, backgroundColor: '#ddd',
        borderRadius: 8
    },
    listingText: { marginLeft: 12, fontSize: 16, color: '#333' },
  listingTitle: { marginLeft:12, fontSize:18, color:'#333' },
